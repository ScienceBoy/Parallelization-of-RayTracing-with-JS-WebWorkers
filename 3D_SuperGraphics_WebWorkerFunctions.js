...
{ 
	this.items = new Array(x);
	for (var i = 0; i < x; i++) this.items[i] = new Array(y); 
	for (var i = 0; i < x; i++) for (var j = 0; j < y; j++) this.items[i][j] = new Array(z); 
}
...
{
	if (!AreaConstants.items[AreaNmbr].RoundedEdges) return {nx: AreaConstants.items[AreaNmbr].nx, ny: AreaConstants.items[AreaNmbr].ny, nz: AreaConstants.items[AreaNmbr].nz,};
	...
	LengthToCenter1 = Math.abs(LengthOfVector(AreaConstants.items[AreaNmbr].CenterX - PosX, AreaConstants.items[AreaNmbr].CenterY - PosY, AreaConstants.items[AreaNmbr].CenterZ - PosZ));
	for (var ii = 0; ii < AreaConstants.items[AreaNmbr].AdjacentAreas.length; ii++) 
	{
		...
		if (i != AreaNmbr && !isNaN(LengthToCenter1) && !isNaN(AreaConstants.items[AreaNmbr].nx) && !isNaN(AreaConstants.items[AreaNmbr].ny) && !isNaN(AreaConstants.items[AreaNmbr].nz) && !isNaN(AreaConstants.items[i].nx) && !isNaN(AreaConstants.items[i].ny) && !isNaN(AreaConstants.items[i].nz)) 
		{
			...
			LengthToCenter2 = Math.abs(LengthOfVector(AreaConstants.items[i].CenterX - PosX, AreaConstants.items[i].CenterY - PosY, AreaConstants.items[i].CenterZ - PosZ));
			...
			if (!isNaN(LengthToCenter2))
			{
				...
				ResultingNx     = ResultingNx + AreaConstants.items[AreaNmbr].nx*(LengthToCenter2/(LengthToCenter1+LengthToCenter2)) + AreaConstants.items[i].nx*(LengthToCenter1/(LengthToCenter1+LengthToCenter2));
				ResultingNy     = ResultingNy + AreaConstants.items[AreaNmbr].ny*(LengthToCenter2/(LengthToCenter1+LengthToCenter2)) + AreaConstants.items[i].ny*(LengthToCenter1/(LengthToCenter1+LengthToCenter2));
				ResultingNz     = ResultingNz + AreaConstants.items[AreaNmbr].nz*(LengthToCenter2/(LengthToCenter1+LengthToCenter2)) + AreaConstants.items[i].nz*(LengthToCenter1/(LengthToCenter1+LengthToCenter2));
				NumberOfValues++;
	...
	return {nx: Returnnx/ReturnLengthOfNormal, ny: Returnny/ReturnLengthOfNormal, nz: Returnnz/ReturnLengthOfNormal,}
	//return AreaConstants.items[AreaNmbr].nx;
}

function GetNxNyNzForAreaWithoutLocation(AreaNmbr)
{
	return {nx: AreaConstants.items[AreaNmbr].nx, ny: AreaConstants.items[AreaNmbr].ny, nz: AreaConstants.items[AreaNmbr].nz,}
}
...
function DoTwoAreasHaveACommonCorner(OriginArea, ComparedArea)
{
	if (OriginArea != ComparedArea)
	{
		for (var OriginCorner = 0; OriginCorner < 3; OriginCorner++)
		{
			...
			{
				if (LengthOfVector(Area.items[OriginArea][OriginCorner][0] - Area.items[ComparedArea][ComparedCorner][0], Area.items[OriginArea][OriginCorner][1] - Area.items[ComparedArea][ComparedCorner][1], Area.items[OriginArea][OriginCorner][2] - Area.items[ComparedArea][ComparedCorner][2])<1e-6)
					return true;
			}
		}
	}
...
function CloneAreaConstants(From, To)
{
	AreaConstants.items[To] = 
	{
		...
		LargestSizeOfAreaInXDirectionMax: AreaConstants.items[From].LargestSizeOfAreaInXDirectionMax,
		LargestSizeOfAreaInYDirectionMax: AreaConstants.items[From].LargestSizeOfAreaInYDirectionMax,
		LargestSizeOfAreaInZDirectionMax: AreaConstants.items[From].LargestSizeOfAreaInZDirectionMax,
		LargestSizeOfAreaInXDirectionMin: AreaConstants.items[From].LargestSizeOfAreaInXDirectionMin,
		LargestSizeOfAreaInYDirectionMin: AreaConstants.items[From].LargestSizeOfAreaInYDirectionMin,
		LargestSizeOfAreaInZDirectionMin: AreaConstants.items[From].LargestSizeOfAreaInZDirectionMin,		
	  ...
	  if (SelectedArea[i] == true)
	  {
		for (j = 0; j < 3; j++) 
		{
			...
			for (ii = 0; ii < AreaIndex; ii++)
			{
				for (jj = 0; jj < 3; jj++)
				{
					if (Math.abs(Area.items[ii][jj][0]-b)<1e-9 && Math.abs(Area.items[ii][jj][1]-c)<1e-9 && Math.abs(Area.items[ii][jj][2]-d)<1e-9)
					{
...
function CalcSizeOfObject()
{
	...
	for (j = 0; j <= NumberOfCompleteObjects; j++)
	{
		...
		for (i = 0; i < AreaIndex; i++) 
		{
			...
			{
				if (xmax < AreaConstants.items[i].LargestSizeOfAreaInXDirectionMax) {xmax = AreaConstants.items[i].LargestSizeOfAreaInXDirectionMax;}
				if (ymax < AreaConstants.items[i].LargestSizeOfAreaInYDirectionMax) {ymax = AreaConstants.items[i].LargestSizeOfAreaInYDirectionMax;}
				if (zmax < AreaConstants.items[i].LargestSizeOfAreaInZDirectionMax) {zmax = AreaConstants.items[i].LargestSizeOfAreaInZDirectionMax;}
				if (xmin > AreaConstants.items[i].LargestSizeOfAreaInXDirectionMin) {xmin = AreaConstants.items[i].LargestSizeOfAreaInXDirectionMin;}
				if (ymin > AreaConstants.items[i].LargestSizeOfAreaInYDirectionMin) {ymin = AreaConstants.items[i].LargestSizeOfAreaInYDirectionMin;}
				if (zmin > AreaConstants.items[i].LargestSizeOfAreaInZDirectionMin) {zmin = AreaConstants.items[i].LargestSizeOfAreaInZDirectionMin;}
			}
		}		
		for (i = 0; i < AreaIndex; i++) 
		{
...
function ShowAllLamps()
{
	...
	do
	{
		...
		for (lamp = 0; lamp < 5; lamp++)
		{
			if (LampMin > LightSourceX.items[lamp]) {LampMin = LightSourceX.items[lamp]};
			if (LampMax < LightSourceX.items[lamp]) {LampMax = LightSourceX.items[lamp]};
		}
...
function ShowAllObjects()
{
	...
	do
	{
		...
		for (i = 0; i < AreaIndex; i++)
		{
			for (j = 0; j < 3; j++)
			{
				if (ObjectMin > AreaOriginal.items[i][j][0]) {ObjectMin = AreaOriginal.items[i][j][0]};
				if (ObjectMax < AreaOriginal.items[i][j][0]) {ObjectMax = AreaOriginal.items[i][j][0]};
			}
		}
		ObjectAverage = (ObjectMax-ObjectMin)/2;
	} while (Math.abs(ObjectMin-ObjectMax) < width)
	MoveX = width/2-ObjectAverage; MoveAll(); MoveX = 0;
	...
	do
	{
		...
		for (i = 0; i < AreaIndex; i++)
		{
			for (j = 0; j < 3; j++)
			{
				if (ObjectMin > AreaOriginal.items[i][j][1]) {ObjectMin = AreaOriginal.items[i][j][1]};
				if (ObjectMax < AreaOriginal.items[i][j][1]) {ObjectMax = AreaOriginal.items[i][j][1]};
			}
		}
		ObjectAverage = (ObjectMax-ObjectMin)/2;
	} while (Math.abs(ObjectMin-ObjectMax) < height)
	MoveY = height/2-ObjectAverage; MoveAll(); MoveY = 0;
	zoomen(0.5);
	ShowAreaOf3DObject();
}
...
function BezierBernsteinApproximation()
{
  try {
	...
	var Area1, Corner1, Distance1, Area2, Corner2, Distance2, Area3, Corner3, Distance3, Area4, Corner4, Distance4;
	var counter = 0;
	var AreaTemp = new Array3D(maxarea, 4, 3);

	//for (i = 0; i < AreaIndex; i++) 
	for (var RandomAreaSelection = 0; RandomAreaSelection < AreaIndex; RandomAreaSelection++) 
	{ 
		...
		if (SelectedArea[i] == true)
		{
			...
			// { // Chosen Corner
				...
				do
				{
					...
					DoCounter++; if (DoCounter == 1000) {Area1 = undefined;}
				}while ((Area1 == i))
				if (Area1 == undefined || Corner1 == undefined) {Area1 = i; Corner1 = ii; Distance1 = 0;}
				...
				do
				{
					CornerOfNeighbour = FindCornerOfNeighbourArea(Area1, Corner1); // FindClosestNeighbourCorner(Area1, Corner1);
					...
					// counter = 0;
					DoCounter++; if (DoCounter == 1000) {Area2 = undefined;}
				...
				// }
				if (Area2 == undefined || Corner2 == undefined) {Area2 = Area1; Corner2 = Corner1; Distance2 = 0;}
				...
				do
				{
					CornerOfNeighbour = FindCornerOfNeighbourArea(Area2, Corner2); // FindClosestNeighbourCorner(Area2, Corner2);
					...
					// counter = 0;
					DoCounter++; if (DoCounter == 1000) {Area3 = undefined;}
				...
				// }
				if (Area3 == undefined || Corner3 == undefined) {Area3 = Area2; Corner3 = Corner2; Distance3 = 0;}
				...
				do
				{
					CornerOfNeighbour = FindCornerOfNeighbourArea(Area3, Corner3); // FindClosestNeighbourCorner(Area3, Corner3);
					...
					// counter = 0;
					DoCounter++; if (DoCounter == 1000) {Area4 = undefined;}
				}while ((Area4 == i) || (Area4 == Area3) || (Area4 == Area2) || (Area4 == Area1))
				...
				if (Area4 == undefined || Corner4 == undefined) {Area4 = Area3; Corner4 = Corner3;  Distance4 = 0;}
				
				DrawPointOfAreaInViews(Area4, Corner4, 4); 
								
				var x0  = Area.items[i][ii][0];          var y0  = Area.items[i][ii][1];          var z0  = Area.items[i][ii][2];
				var x1  = Area.items[Area1][Corner1][0]; var y1  = Area.items[Area1][Corner1][1]; var z1  = Area.items[Area1][Corner1][2];
				var x2  = Area.items[Area2][Corner2][0]; var y2  = Area.items[Area2][Corner2][1]; var z2  = Area.items[Area2][Corner2][2];
				var x3  = Area.items[Area3][Corner3][0]; var y3  = Area.items[Area3][Corner3][1]; var z3  = Area.items[Area3][Corner3][2];
				var x4  = Area.items[Area4][Corner4][0]; var y4  = Area.items[Area4][Corner4][1]; var z4  = Area.items[Area4][Corner4][2];

				var t = Distance1/(Distance1+Distance2+Distance3+Distance4);
				var B04 = Math.pow(1-t,4);
				var B14 = 4*t*Math.pow(1-t,3);
				var B24 = 6*t*t*Math.pow(1-t,2);
				var B34 = 4*t*t*t*(1-t);
				var B44 = Math.pow(t,4);
				var NewCorner1x = x0*B04 + x1*B14 + x2*B24 + x3*B34 + x4*B44;
				var NewCorner1y = y0*B04 + y1*B14 + y2*B24 + y3*B34 + y4*B44;
				var NewCorner1z = z0*B04 + z1*B14 + z2*B24 + z3*B34 + z4*B44;

				t = (Distance1+Distance2)/(Distance1+Distance2+Distance3+Distance4);
				// B03 = Math.pow(1-t,3);
				...
				B34 = 4*t*t*t*(1-t);
				B44 = Math.pow(t,4);
				var NewCorner2x = x0*B04 + x1*B14 + x2*B24 + x3*B34 + x4*B44;
				var NewCorner2y = y0*B04 + y1*B14 + y2*B24 + y3*B34 + y4*B44;
				var NewCorner2z = z0*B04 + z1*B14 + z2*B24 + z3*B34 + z4*B44;

				t = (Distance1+Distance2+Distance3)/(Distance1+Distance2+Distance3+Distance4);
				B04 = Math.pow(1-t,4);
				B14 = 4*t*Math.pow(1-t,3);
				B24 = 6*t*t*Math.pow(1-t,2);
				B34 = 4*t*t*t*(1-t);
				B44 = Math.pow(t,4);
				var NewCorner3x = x0*B04 + x1*B14 + x2*B24 + x3*B34 + x4*B44;
				var NewCorner3y = y0*B04 + y1*B14 + y2*B24 + y3*B34 + y4*B44;
				var NewCorner3z = z0*B04 + z1*B14 + z2*B24 + z3*B34 + z4*B44;
				
				t = (Distance1+Distance2+Distance3+Distance4)/(Distance1+Distance2+Distance3+Distance4);
				B04 = Math.pow(1-t,4);
				B14 = 4*t*Math.pow(1-t,3);
				B24 = 6*t*t*Math.pow(1-t,2);
				B34 = 4*t*t*t*(1-t);
				B44 = Math.pow(t,4);
				var NewCorner4x = x0*B04 + x1*B14 + x2*B24 + x3*B34 + x4*B44;
				var NewCorner4y = y0*B04 + y1*B14 + y2*B24 + y3*B34 + y4*B44;
				var NewCorner4z = z0*B04 + z1*B14 + z2*B24 + z3*B34 + z4*B44;
				...
				console.log("0: ",Area.items[i][ii][0],Area.items[i][ii][1],Area.items[i][ii][2]);

				console.log("1 alt: ",Area.items[Area1][Corner1][0],Area.items[Area1][Corner1][1],Area.items[Area1][Corner1][2]);
				console.log("1 neu: ",NewCorner1x,NewCorner1y,NewCorner1z);

				console.log("2 alt: ",Area.items[Area2][Corner2][0],Area.items[Area2][Corner2][1],Area.items[Area2][Corner2][2]);
				console.log("2 neu: ",NewCorner2x,NewCorner2y,NewCorner2z);

				console.log("3 alt: ",Area.items[Area3][Corner3][0],Area.items[Area3][Corner3][1],Area.items[Area3][Corner3][2]);
				console.log("3 neu: ",NewCorner3x,NewCorner3y,NewCorner3z);
				
				console.log("4 alt: ",Area.items[Area4][Corner4][0],Area.items[Area4][Corner4][1],Area.items[Area4][Corner4][2]);
				...
				if (LengthOfVector(NewCorner1x-NewCorner2x,NewCorner1y-NewCorner2y,NewCorner1z-NewCorner2z)>1e-9 && LengthOfVector(NewCorner1x-NewCorner3x,NewCorner1y-NewCorner3y,NewCorner1z-NewCorner3z)>1e-9 && LengthOfVector(NewCorner1x-NewCorner4x,NewCorner1y-NewCorner4y,NewCorner1z-NewCorner4z)>1e-9 && LengthOfVector(NewCorner2x-NewCorner3x,NewCorner2y-NewCorner3y,NewCorner2z-NewCorner3z)>1e-9 && LengthOfVector(NewCorner2x-NewCorner4x,NewCorner2y-NewCorner4y,NewCorner2z-NewCorner4z)>1e-9 && LengthOfVector(NewCorner3x-NewCorner4x,NewCorner3y-NewCorner4y,NewCorner3z-NewCorner4z)>1e-9)
				{
					...
					for (inew = 0; inew < AreaIndex; inew++)
					{
						...
						{	
							...
							if (NewCorner1x != NaN && NewCorner1y != NaN && NewCorner1z != NaN && Math.abs(Tempx - AreaTemp.items[Area1][Corner1][0])<1e-9 && Math.abs(Tempy - AreaTemp.items[Area1][Corner1][1])<1e-9 && Math.abs(Tempz - AreaTemp.items[Area1][Corner1][2])<1e-9)
							{
							...
							if (NewCorner2x != NaN && NewCorner2y != NaN && NewCorner2z != NaN && Math.abs(Tempx - AreaTemp.items[Area2][Corner2][0])<1e-9 && Math.abs(Tempy - AreaTemp.items[Area2][Corner2][1])<1e-9 && Math.abs(Tempz - AreaTemp.items[Area2][Corner2][2])<1e-9)
							{
								Area.items[inew][jnew][0] = NewCorner2x;
								Area.items[inew][jnew][1] = NewCorner2y;
								Area.items[inew][jnew][2] = NewCorner2z;
							}
							if (NewCorner3x != NaN && NewCorner3y != NaN && NewCorner3z != NaN && Math.abs(Tempx - AreaTemp.items[Area3][Corner3][0])<1e-9 && Math.abs(Tempy - AreaTemp.items[Area3][Corner3][1])<1e-9 && Math.abs(Tempz - AreaTemp.items[Area3][Corner3][2])<1e-9)
							{
								Area.items[inew][jnew][0] = NewCorner3x;
								Area.items[inew][jnew][1] = NewCorner3y;
								Area.items[inew][jnew][2] = NewCorner3z;
							}
							if (NewCorner4x != NaN && NewCorner4y != NaN && NewCorner4z != NaN && Math.abs(Tempx - AreaTemp.items[Area4][Corner4][0])<1e-9 && Math.abs(Tempy - AreaTemp.items[Area4][Corner4][1])<1e-9 && Math.abs(Tempz - AreaTemp.items[Area4][Corner4][2])<1e-9)
							{
...
function FindClosestNeighbourCorner(OriginArea, OriginCorner)
{
	...
	for (m0 = 0; m0 < Math.min(36,AreaIndex); m0++) {PointDistance.items[m0] = 1e6;}
	...
	{ // Compared Area	
		...
		{ // Compared Corner
			...
			Distance = LengthOfVector(Math.abs(Area.items[OriginArea][OriginCorner][0]-Area.items[ComparedArea][ComparedCorner][0]), Math.abs(Area.items[OriginArea][OriginCorner][1]-Area.items[ComparedArea][ComparedCorner][1]), Math.abs(Area.items[OriginArea][OriginCorner][2]-Area.items[ComparedArea][ComparedCorner][2]));
			for (m2 = 0; m2 < Math.min(36,AreaIndex); m2++)
			{
				if (Distance > 0 && Distance < PointDistance.items[m2] && ClosestCornerFound == false && HaveTwoAreasCommonCorner(OriginArea,ComparedArea) == true) 
				{
...
function HaveTwoAreasCommonCorner(OriginArea,OriginCorner,ComparedArea)
{
	...
	// {	
		for (var l = 0; l < 3; l++)
		{
			...
			if (OriginArea != ComparedArea && LengthOfVector(Area.items[OriginArea][OriginCorner][0] - Area.items[ComparedArea][l][0], Area.items[OriginArea][OriginCorner][1] - Area.items[ComparedArea][l][1], Area.items[OriginArea][OriginCorner][2] - Area.items[ComparedArea][l][2])<1e-6)
			{
				// console.log("Ecke: ",l);
				return true;
			}
...
function FindCornerOfNeighbourArea(AreaOrigin, CornerOrigin)
{
	...
	for (var i = 0; i < 100; i++)
	{
		...
		if (ChoseArea != AreaOrigin && HaveTwoAreasCommonCorner(AreaOrigin, CornerOrigin, ChoseArea) == true)
		{
			// var counter = 0;
			// do
			// { 
			...
			// if (counter == 1000) {ChoseArea = AreaOrigin; ChoseCorner = CornerOrigin;}
 			var Distance0 = LengthOfVector(Area.items[AreaOrigin][CornerOrigin][0]-Area.items[ChoseArea][0][0], Area.items[AreaOrigin][CornerOrigin][1]-Area.items[ChoseArea][0][1], Area.items[AreaOrigin][CornerOrigin][2]-Area.items[ChoseArea][0][2]);
 			var Distance1 = LengthOfVector(Area.items[AreaOrigin][CornerOrigin][0]-Area.items[ChoseArea][1][0], Area.items[AreaOrigin][CornerOrigin][1]-Area.items[ChoseArea][1][1], Area.items[AreaOrigin][CornerOrigin][2]-Area.items[ChoseArea][1][2]);
 			var Distance2 = LengthOfVector(Area.items[AreaOrigin][CornerOrigin][0]-Area.items[ChoseArea][2][0], Area.items[AreaOrigin][CornerOrigin][1]-Area.items[ChoseArea][2][1], Area.items[AreaOrigin][CornerOrigin][2]-Area.items[ChoseArea][2][2]);
 			// console.log(Distance0, Distance1, Distance2);
			if (Distance0 < 1e-6) {if (Distance1 < Distance2) {return [ChoseArea, 1, Distance1];} else {return [ChoseArea, 2, Distance2];}}
			if (Distance1 < 1e-6) {if (Distance0 < Distance2) {return [ChoseArea, 0, Distance0];} else {return [ChoseArea, 2, Distance2];}} 
			if (Distance2 < 1e-6) {if (Distance0 < Distance1) {return [ChoseArea, 1, Distance1];} else {return [ChoseArea, 0, Distance0];}} 
		}
	}
...
function ColorRedOfAreaAtThisPoint(objectNr, x,y,z)
{
	...
	{
		var xtemp = x; var ytemp = y; var ztemp = z;
		var RadFromGrad = 2*Math.PI/360;
		var y_rot = ytemp * Math.cos(RadFromGrad*AreaConstants.items[objectNr].TotalAngleX) - ztemp * Math.sin(RadFromGrad*AreaConstants.items[objectNr].TotalAngleX);
		var z_rot = ytemp * Math.sin(RadFromGrad*AreaConstants.items[objectNr].TotalAngleX) + ztemp * Math.cos(RadFromGrad*AreaConstants.items[objectNr].TotalAngleX);
		ytemp = y_rot; 
		ztemp = z_rot;
		var x_rot = xtemp * Math.cos(RadFromGrad*AreaConstants.items[objectNr].TotalAngleY) + ztemp * Math.sin(RadFromGrad*AreaConstants.items[objectNr].TotalAngleY); 
		var z_rot =-xtemp * Math.sin(RadFromGrad*AreaConstants.items[objectNr].TotalAngleY) + ztemp * Math.cos(RadFromGrad*AreaConstants.items[objectNr].TotalAngleY); 
		xtemp = x_rot; 
		ztemp = z_rot;
		var x_rot = xtemp * Math.cos(RadFromGrad*AreaConstants.items[objectNr].TotalAngleZ) - ytemp * Math.sin(RadFromGrad*AreaConstants.items[objectNr].TotalAngleZ); 
		var y_rot = xtemp * Math.sin(RadFromGrad*AreaConstants.items[objectNr].TotalAngleZ) + ytemp * Math.cos(RadFromGrad*AreaConstants.items[objectNr].TotalAngleZ); 
		xtemp = x_rot; 
		ytemp = y_rot;
		if (((Math.floor(xtemp)+Math.floor(ytemp)+Math.floor(ztemp)) % 2) == 0) return 1; else return 0.7;
	}
	else if (AreaConstants.items[objectNr].TypeOfColor == "Color") {return AreaConstants.items[objectNr].ColorOfObjectRed;}
}
...
function ColorGreenOfAreaAtThisPoint(objectNr, x,y,z)
{
	if (AreaConstants.items[objectNr].TypeOfColor == "CheckBoard") 
	{
		...
		xtemp = x_rot; 
		ytemp = y_rot;
		if (((Math.floor(xtemp)+Math.floor(ytemp)+Math.floor(ztemp)) % 2) == 0) return 0; else return 0.7;
	}
	else if (AreaConstants.items[objectNr].TypeOfColor == "Color") {return AreaConstants.items[objectNr].ColorOfObjectGreen;}
}
...
function ColorBlueOfAreaAtThisPoint(objectNr, x,y,z)
{
	...
	else if (AreaConstants.items[objectNr].TypeOfColor == "Color") {return AreaConstants.items[objectNr].ColorOfObjectBlue;}
}

function ApplyCheckerBoard()
{
	for (var i = 0; i < AreaIndex; i++)
	if (SelectedArea[i] == true) AreaConstants.items[i].TypeOfColor = "CheckBoard";
}

function ApplyPlainColor()
{
	for (var i = 0; i < AreaIndex; i++)
	if (SelectedArea[i] == true) AreaConstants.items[i].TypeOfColor = "Color";
}

function SecToHourMinSec(value)
{
	var hours   = Math.floor(value / 3600); value = value - hours * 3600;
	var minutes = Math.floor(value / 60); 
	var seconds = value - minutes * 60;
	var finalTime = parseInt(hours,10)+":";
	if (parseInt(minutes,10)<10) finalTime = finalTime+"0"+parseInt(minutes,10)+":"; else finalTime = finalTime+parseInt(minutes,10)+":";
	if (parseInt(seconds,10)<10) finalTime = finalTime+"0"+parseInt(seconds,10); else finalTime = finalTime+parseInt(seconds,10);
	return finalTime;
}